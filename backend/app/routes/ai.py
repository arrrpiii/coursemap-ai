...
from app.ai_flows.questions import generate_questions as ai_generate_questions
from app.ai_flows.sample_paper import generate_sample_paper as ai_generate_sample_paper
...
@router.get("/{course_id}/nodes/{node_id}/questions/history")
async def list_previous_questions(
    course_id: str,
    node_id: str,
    limit: int = 20,
    current_user: dict = Depends(get_current_user),
    ...
    cursor = (
        db.ai_outputs.find(
            {"courseId": course_oid, "nodeId": node_oid, "type": "questions"}
        )
        .sort("createdAt", -1)
...
async def generate_sample_paper(
    ...
    try:
        result = ai_generate_sample_paper(
...
@router.get("/{course_id}/sample-paper/history")
async def list_previous_sample_papers(
    course_id: str,
    limit: int = 20,
    current_user: dict = Depends(get_current_user),
):
    ...
    cursor = (
        db.ai_outputs.find(
            {"courseId": course_oid, "type": "sample_paper"}
        )
        .sort("createdAt", -1)
        .limit(max(1, min(limit, 50)))
    )
